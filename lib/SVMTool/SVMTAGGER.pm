...
print "Going supervised on $line\n";
                  $supervised = 0;
               }
               else {
...
sub enrich_dictionary
{
   ...
   $DICT = new IO::File("> $dict") or die "Couldn't open output file: $dict\n";
...
sub pos_tag_sample {
    ...
    my $rattribs = ATTGEN::generate_features($rattribw, $rdict, $sinfo, $M->{mode}, $unknown ? $M->{fsu} : $M->{fsk});
...
sub analyze_sample
{
    ...
    if ($mode == $SVMTAGGER::Mspecial) {
       if (any_unknown($rwindow, $M->{dict})) {
          ($pos, $pred, $rclsf, $Ftime, $Ctime) = pos_tag_sample($rwindow, $M->{dict}, $w, $sinfo, $M->{dict}->unknown_word($w), $all, $potser, $M2, $direction, $verbose);
       }
       else {
          ($pos, $pred, $rclsf, $Ftime, $Ctime) = pos_tag_sample($rwindow, $M->{dict}, $w, $sinfo, $M->{dict}->unknown_word($w), $all, $potser, $M1, $direction, $verbose);
       }
    }
    else { #Mdefault
       ($pos, $pred, $rclsf, $Ftime, $Ctime) = pos_tag_sample($rwindow, $M->{dict}, $w, $sinfo, $M->{dict}->unknown_word($w), $all, $potser, $M1, $direction, $verbose);
...
sub process_sample_merged {
    ...
    #is the core item active?
    if ($rwindow->active()) {
       ($w, $pos, $pred, $rclsf, $Ftime, $Ctime) = analyze_sample($rwindow, $M, $sinfo, $M1, $M2, $mode, $direction, $verbose);
       # --> feed back always
...
sub do_greedy_tagging
{
    ...
    while ($iter < $SIZE) {
       ...
       if ($word ne "") { # don't process emtpy lines
          shift(@{$columns});
          my ($w, $pos, $pred, $rclsf, $SFtime, $SCtime) = process_sample_merged($rwindow, $word, $wordpos, $M, $M1, $M2, $sinfo, $mode, $direction, $columns, $verbose);
    ...
    while ($i > 0) { #process last words in the sliding window
       my ($w, $pos, $pred, $rclsf, $SFtime, $SCtime) = process_sample_merged($rwindow, $COMMON::emptyword, $COMMON::emptypos, $M, $M1, $M2, $sinfo, $mode, $direction, 0, $verbose);
       if (defined($SFtime)) { $Ftime += $SFtime->[1]; }
       if (defined($SCtime)) { $Ctime += $SCtime->[1]; }
       if ($w ne "") { # empty words don't get tagged
...
sub compute_scores_old {
    ...
    foreach my $tj (@{$possible}) {
       ...
       if ($i == 0) { # no left context is available
          my ($rclsf, $ftime, $ctime) = compute_tagging($rwindow, $M, $sinfo, $rdict, $direction, $rambp, $runkp, $verbose);
          #APPLY softmax
...
sub do_tagging
{
    ...
    if ($mode == $SVMTAGGER::Mviterbi) {
       return do_viterbi_tagging($M, $verbose, $direction, $input_LR, $M1);
    }
    else {  #Mdefault or Mspecial
       return do_greedy_tagging($M, $verbose, $direction, $input_LR, $M1, $M2, $mode);
    }
}
...
sub do_tagging_LRL_1P
{
    ...
    if ($direction eq $COMMON::lrmode) {      # LEFT-TO-RIGHT
       if ($M2 == 0) {
          ($OUT_LR, $GscoreLR, $Ftime, $Ctime) = do_tagging($M, $verbose, $COMMON::lrmode, $input_LR, $M1->{LR}, 0, $mode);
       }
       else {
          ($OUT_LR, $GscoreLR, $Ftime, $Ctime) = do_tagging($M, $verbose, $COMMON::lrmode, $input_LR, $M1->{LR}, $M2->{LR}, $mode);
       }
       return($OUT_LR, $Ftime, $Ctime);
    }
    elsif ($direction eq $COMMON::rlmode) {      # RIGHT-TO-LEFT
       if ($M2 == 0) {
          ($OUT_RL, $GscoreRL, $Ftime, $Ctime) = do_tagging($M, $verbose, $COMMON::rlmode, $input_LR, $M1->{RL}, 0, $mode);
       }
       else {
          ($OUT_RL, $GscoreRL, $Ftime, $Ctime) = do_tagging($M, $verbose, $COMMON::rlmode, $input_LR, $M1->{RL}, $M2->{RL}, $mode);
       }
       return($OUT_RL, $Ftime, $Ctime);
    }
    elsif ($direction eq $COMMON::lrlmode) {      #both LEFT-TO-RIGHT and RIGHT-TO-LEFT
       if ($M2 == 0) {
          ($OUT_LR, $GscoreLR, $FtimeLR, $CtimeLR) = do_tagging($M, $verbose, $COMMON::lrmode, $input_LR, $M1->{LR}, 0, $mode);
          ($OUT_RL, $GscoreRL, $FtimeRL, $CtimeRL) = do_tagging($M, $verbose, $COMMON::rlmode, $input_LR, $M1->{RL}, 0, $mode);
          $Ftime = $FtimeLR + $FtimeRL;
          $Ctime = $CtimeLR + $CtimeRL;
       }
       else {
          ($OUT_LR, $GscoreLR, $FtimeLR, $CtimeLR) = do_tagging($M, $verbose, $COMMON::lrmode, $input_LR, $M1->{LR}, $M2->{LR}, $mode);
          ($OUT_RL, $GscoreRL, $FtimeRL, $CtimeRL) = do_tagging($M, $verbose, $COMMON::rlmode, $input_LR, $M1->{RL}, $M2->{RL}, $mode);
          $Ftime = $FtimeLR + $FtimeRL;
          $Ctime = $CtimeLR + $CtimeRL;
       }
...
sub do_tagging_LRL_2P
{
    ...
    my ($auxl, $Ftime, $Ctime) = do_tagging_LRL_1P($M, $verbose, $direction, $input_LR, $M1, $M2, $mode1);
    if ($verbose > $COMMON::verbose2) { print STDERR "2nd PASS\n"; }
    return (do_tagging_LRL_1P($M, $verbose, $direction, $auxl, $M3, $M4, $mode2), $Ftime, $Ctime);
}
...
sub SVMT_tag
{
   ...
   if (scalar(@{$INPUT_LR}) > 0) { # list isn't empty
      if (($strategy == $COMMON::st0) or ($strategy == $COMMON::st3) or ($strategy == $COMMON::st4)) {
         ...
         ($out, $Ftime, $Ctime) = do_tagging_LRL_2P($M, $verbose, $direction, $INPUT_LR, $M->{P1}, 0, $M->{P2}, 0, $SVMTAGGER::Mdefault, $SVMTAGGER::Mdefault);
      }
...
sub learn_known
{
    ...
    if ($remakeF) {
       if ($mode == $COMMON::mode4) {
          SVMTAGGER::do_attribs_kn_unk($model, $trainset, $smplset, $direction, $mode, $config, $report, $verbose);
       }
       else {
          SVMTAGGER::do_attribs_kn($trainset, $rdict, $smplset, $direction, $mode, $config, $report, $verbose);
       }
...
sub learn_unknown
{
    ...
    if ($remakeF) {
       SVMTAGGER::do_attribs_unk($model, $trainset, $rdict, $unksmplset, $direction, $mode, $config, $report, $verbose);
    ...
    while ($depth < $MAX_DEPTH){
       ...
       while (((sprintf("%.4f", $MAX_C - $C) + 0) >= 0) && ($C >= 0)) {
          ...
          if (($depth == 0) and ($i == 0)) { #BUILDING COMPLEMENTARY MODELS - only if they didn't exist
             ...
             if ($unknown) { #train complementary model (one not being tuned) outside the loop
                if (!(-e "$model.$modext.$dirext.$COMMON::MRGEXT")) {
                   SVMTAGGER::learn_known($model, $trainset, $smplset, $smplmap, $rdict, $mode, $modext, $direction, $dirext, $config, 1, $report, $verbose);
                ...
                if (!(-e "$model.$COMMON::unkext.$modext.$dirext.$COMMON::MRGEXT")) {
                   SVMTAGGER::learn_unknown($model, $trainset, $unksmplset, $unksmplmap, $rdict, $mode, $modext, $direction, $dirext, $config, 1, $report, $verbose);
                   SVMTAGGER::merge_models($model, $modext, $dirext, $smplmap, $unksmplmap, $config, $COMMON::MRGEXT, !$unknown, $report, $verbose);
                }
             }
	  }
          ...
          if ($unknown) {
             ...
             SVMTAGGER::learn_unknown($model, $trainset, $unksmplset, $unksmplmap, $rdict, $mode, $modext, $direction, $dirext, $config, $remakeF, $report, $verbose);
 	  }
          else {
             $config->{Ck} = $C;
             $remakeF = (!(-e "$model.$modext.$dirext.$COMMON::smplext.$COMMON::DSFEXT"));
             SVMTAGGER::learn_known($model, $trainset, $smplset, $smplmap, $rdict, $mode, $modext, $direction, $dirext, $config, $remakeF, $report, $verbose);
	  }
...
sub adjust_C_CV
{
    ...
    while ($depth < $MAX_DEPTH){
	  ...
	  my $acckn; my $accamb; my $accunk; my $accuracy;
          ...
          if ($unknown) {
             $config->{Cu} = $C;
             ($acckn, $accamb, $accunk, $accuracy) = do_CV($config, $mode, $direction, $trainset, $nfolders, $rdict, 1, $remakeFLDS, $report, $verbose);
 	  }
          else {
             $config->{Ck} = $C;
             ($acckn, $accamb, $accunk, $accuracy) = do_CV($config, $mode, $direction, $trainset, $nfolders, $rdict, 0, $remakeFLDS, $report, $verbose);
	  }
          $remakeFLDS = 0;
...
sub do_CV
{
    ...
    while ($i < $nfolders) {
	  ...
	  if (!(-e "$fmodel.$COMMON::unkext.$modext.$dirext.$COMMON::MRGEXT")) { $fwhich = 2; }
       ...
       SVMTAGGER::build_SVMT($fmodel, $ftrainset, $frdict, $mode, $direction, $config, $fwhich, ($fwhich != 1), ($fwhich != 0), $report, $verbose);
...
sub SVMT_learn
{
    ...
    if ($doCK or $doCU) { #C PARAMETER TUNING
	  ...
	  if ($config->{valset} ne "") {
	     ...
	     if ($doCU) {
                ($Cu, $remakeFU) = SVMTAGGER::adjust_C($model, $trainset, $rdict, $mode, $direction, $config, $CUTopt, $config->{valset}, 1, $report, $verbose); #TUNE UNKNOWN WORDS
                $config->{Cu} = $Cu;
	     }
	  }
          else {
	     if ($verbose) {
          ...
          if ($doCK) {
             ($Ck, $remakeFLDS) = SVMTAGGER::adjust_C_CV($model, $config->{trainset}, $rdict, $NFOLDERS, $mode, $direction, $config, $CKTopt, 0, $remakeFLDS, $report, $verbose); #TUNE KNOWN WORDS
             $config->{Ck} = $Ck;
	  }
	  if ($doCU) {
             ($Cu, $remakeFLDS) = SVMTAGGER::adjust_C_CV($model, $config->{trainset}, $rdict, $NFOLDERS, $mode, $direction, $config, $CUTopt, 1, $remakeFLDS, $report, $verbose); #TUNE UNKNOWN WORDS
             $config->{Cu} = $Cu;
	  }
       }
    ...
    if ($TTYPE ne "CVT") { #TRADITIONAL 1-fold
       ...
       SVMTAGGER::build_SVMT($model, $trainset, $rdict, $mode, $direction, $config, 2, $remakeFK, $remakeFU, $report, $verbose);
	  ...
	  my ($acckn, $accamb, $accunk, $accuracy) = test_SVMT($model, $strategy, $direction, $config->{testset}, "$model.TEST.T$strategy.$direction", $report, $verbose);
       ...
       my ($acckn, $accamb, $accunk, $accuracy) = do_CV($config, $mode, $direction, $config->{trainset}, $NFOLDERS, $rdict, 2, $remakeFLDS, $report, $verbose);
       if ($config->{rmfiles}) { system "rm -f $config->{model}.$COMMON::FOLDEXT*"; }
       if ($verbose) {
         COMMON::report($report, "---------------------------------------------------------------------------------------------\n---------------------------------------------------------------------------------------------\nBUILDING MODELS ON WHOLE CORPUS <".$trainset.">\nC PARAMETER [MODE = $mode :: DIRECTON = $direction]:   KNOWN [ C = ".$config->{Ck}." ] :: UNKNOWN [ C = ".$config->{Cu}." ]\n---------------------------------------------------------------------------------------------\n---------------------------------------------------------------------------------------------\n"); 
       }
       SVMTAGGER::build_SVMT($model, $trainset, $rdict, $mode, $direction, $config, 2, 1, 1, $report, $verbose);
    }
